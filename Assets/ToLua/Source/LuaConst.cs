...
{
	// 外部工程资源路径的外部文件地址
	public static System.Func<string> outProjectLuaDir;
	static string _luaDir = "";
	public static string luaDir{
		get{
			if (outProjectLuaDir != null) {
				_luaDir = outProjectLuaDir ();
			}

			if (string.IsNullOrEmpty (_luaDir)) {
				_luaDir = Application.dataPath + "/GameFrame/Lua";
			}
			return _luaDir;
		}
	}
